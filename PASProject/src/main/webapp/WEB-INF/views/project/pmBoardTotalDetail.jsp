...
<script>
	$(function(){
		...
		$('#go_list').on('click',function(){
			...
			location.href='/pas/project/pmBoardTotalList?bm_Num='+bm_Num;
		})
		
		$('#go_update').on('click',function(){
			if(mem_Email==login_Eem_Email){
				location.href='/pas/project/pmBoardTotalUpdate?bt_Num='+bt_Num;
			}else{
				alert("본인 게시글이 아닙니다.")
			}
		});
		...
		$('#go_delete').on('click',function(){
			if(mem_Email==login_Eem_Email){
				location.href='/pas/project/pmBoardTotalDelte?bt_Num='+bt_Num;
			}else{
				alert("본인 게시글이 아닙니다.")
			}
		})
